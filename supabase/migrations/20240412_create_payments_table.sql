...
(
    ...
    reservation_id UUID NOT NULL REFERENCES public.reservations
(
    id
) ON DELETE CASCADE,
    amount DECIMAL
...
CREATE
POLICY "Admins can do everything with payments" ON public.payments
  FOR ALL
  TO authenticated
...
CREATE
POLICY "Designers can view and create payments" ON public.payments
  FOR
SELECT
    TO authenticated
...
CREATE
POLICY "Designers can insert payments" ON public.payments
  FOR INSERT
  TO authenticated
...
CREATE
POLICY "Tattoo artists can view payments for their reservations" ON public.payments
  FOR
SELECT
    TO authenticated
    USING (
...
CREATE
POLICY "Info users can view payments for their referred customers" ON public.payments
  FOR
SELECT
    TO authenticated
    USING (
