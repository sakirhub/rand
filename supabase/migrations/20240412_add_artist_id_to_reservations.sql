...
CREATE
POLICY "Admins can do everything with artist_id" ON public.reservations
  FOR ALL
  TO authenticated
...
CREATE
POLICY "Designers can view and set artist_id" ON public.reservations
  FOR
SELECT
    TO authenticated
...
CREATE
POLICY "Designers can update artist_id" ON public.reservations
  FOR
UPDATE
    TO authenticated
    USING (auth.jwt() ->> 'role' = 'designer')
...
CREATE
POLICY "Artists can view their own reservations" ON public.reservations
  FOR
SELECT
    TO authenticated
    USING (
